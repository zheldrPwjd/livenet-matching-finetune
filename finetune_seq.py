...
def sobel_xy_loss(pred, gt):
    """Sobel x, y 분리해서 각각 L1 (edge loss)"""
...
def parse_config():
    ...
    parser.add_argument("-lam_perc", type=float, default=0.05)   # perceptual (ft_sobel_perc와 동일)
    parser.add_argument("-lam_fft",  type=float, default=0.1)    # frequency (새로 추가)
    parser.add_argument("-epochs",   type=int,   default=30)     # 짧게 - 이미 sobel+perc는 학습됨
...
def main():
    ...
    logger.info(f"SEQUENTIAL FT (ft_sobel_perc -> +hf_branch +FFT) | "
                 f"lam={lam}, lam_sob={lam_sob}, lam_perc={lam_perc}, lam_fft={lam_fft}, epochs={args.epochs}")

    train_dataloader, _ = get_data(opt)

    generator, refiner = get_model(opt['model'], device=opt['device'])

    # ===== ft_sobel_perc 체크포인트에서 시작 (hf_branch/fusion은 strict=False로 랜덤 초기화) =====
    start_gen = r'C:\Users\user\Desktop\17\repos\LIVENET\checkpoints\ft_sobel_perc0.05\gen_epoch49.pth'
    start_ref = r'C:\Users\user\Desktop\17\repos\LIVENET\checkpoints\ft_sobel_perc0.05\ref_epoch49.pth'
    ...
    generator = load_w(generator, start_gen)
    refiner   = load_w(refiner,   start_ref)
    logger.info("ft_sobel_perc 가중치 로드 완료 (hf_branch/fusion 랜덤 초기화)")
    ...
    save_dir = fr'C:\Users\user\Desktop\17\repos\LIVENET\checkpoints\ft_seq_perc{lam_perc}_hf_fft{lam_fft}'
    ...
    for epoch in range(args.epochs):
        ...
        # 10epoch마다 이전 체크포인트 정리 (디스크 절약, 최근 10개만 유지)
        if epoch >= 10:
            old_epoch = epoch - 10
    ...
    logger.info("SEQUENTIAL FT done!")
